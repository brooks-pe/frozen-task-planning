    ...
    <CollapsibleFilterSection highContrast>
      <div className="content-stretch flex gap-[12px] items-end relative shrink-0" data-name="left-filters">
        ...
        <div className="flex items-center h-[32px] px-[4px] relative self-end">
          <div className="w-[1px] h-full bg-[#e0e1e6]" />
        </div>
        {/* View Radio Filter */}
        <div className="flex flex-col gap-[8px] items-start shrink-0">
...
function StatusTabs({ activeTab, onTabChange }: { activeTab: 'planning' | 'ready' | 'inProgress'; onTabChange: (tab: 'planning' | 'ready' | 'inProgress') => void }) {
  ...
  return (
    <div className="flex items-center gap-0 border border-solid border-[#e0e1e6] rounded-[4px] overflow-hidden" data-name="status-tabs">
      {tabs.map((tab) => {
        ...
        return (
          <button
            ...
            className={`flex items-center gap-[8px] h-[36px] px-[16px] cursor-pointer border-none transition-colors outline-none focus-visible:ring-[3px] focus-visible:ring-[rgba(0,75,114,0.35)] ${
              isActive
...
function ActivityParentRow({ parent, expandAll }: { parent: ActivityParent; expandAll: boolean }) {
  ...
  return (
    <>
      {/* Parent row */}
      <div
        className={`w-full border-b border-[#e0e1e6] hover:bg-[#fafafa] transition-colors grid ${isExpanded ? 'bg-[#fafafa]' : ''}`}
        style={{
...
function FundingSourceChildRow({ child }: { child: FundingSourceChild }) {
  return (
    <div
      className="w-full border-b border-[#e0e1e6] hover:bg-[#fafafa] transition-colors grid bg-white relative"
      style={{
        gridTemplateColumns: FWP_GRID,
        borderLeft: '4px solid #e0e1e6',
      }}
    >
